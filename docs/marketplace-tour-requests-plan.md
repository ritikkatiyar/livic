...
|---|---|---|
| `GET` | `/api/v1/marketplace/properties/{propertyId}/tour-requests?filter=PENDING\|UPCOMING\|PAST&page=&size=` | `@PreAuthorize hasPermission(#propertyId, 'LEASE_VIEW')` |
| `GET` | `/api/v1/marketplace/properties/{propertyId}/tour-requests/summary` | same; returns `{pending, upcoming}` for the tab badge |
| `POST` | `/api/v1/marketplace/tour-requests/{leadId}/approve` | load lead → `authorizationService.hasPermission(lead.propertyId, 'LEASE_UPDATE')` |
| `POST` | `/api/v1/marketplace/tour-requests/{leadId}/reject` body `{note?}` | same |
